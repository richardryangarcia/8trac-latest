...
contract("Artist", accounts => {
  ...
  beforeEach(async () => {
    instance = await Artist.new(
      "Kanye West",
      "kw1",
      18,
      "rap/r&b",
      "dat yeezus boi from the Chi",
      "the Chi",
      "QWDSDFGSDGHRSEERWER"
    );
  ...
  describe("create crowdfund project functions", async () => {
    it("create a project ", async () => {
      ...
      let response = await instance.createProject(
        1,
        cap,
        "Graduation",
        "3rd full length album",
        "QLKJHSDAFLIUEIULHLJKHKLJ"
      );
    ...
    it("create a project ", async () => {
      ...
      let response = await instance.projectAddresses();
  ...
  describe("artist store methods", () => {
    ...
    it("make merch unavailable then available", async () => {
      await instance.addMerch("tshirt", "dope tshirt", 20, 15.0, "");
