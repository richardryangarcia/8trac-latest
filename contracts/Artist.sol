...
contract Artist is ERC20Detailed, ERC20Mintable, ERC20Pausable {
    ...
    function projectAddresses() public view returns (address[] memory) {
        uint pCount = projectCount;
        address[] memory projectsAddresses = new address[](pCount);
        for (uint i = 0; i < pCount; i++ ){
            projectsAddresses[i] = projects[i];
        }
        return projectsAddresses;
    }
